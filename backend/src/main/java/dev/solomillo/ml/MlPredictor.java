...
public class MlPredictor {
    ...
    public MlPredictor(
        ModeloPredictivoRepository m,
        PartidoRepository p,
        PrediccionRepository pr,
        EstadisticaJugadorRepository ej
    ) {
        this.modeloRepo = m;
        this.partidoRepo = p;
    ...
    @Transactional
    public Map<String, Object> predecirResultado(Long partidoId)
        throws Exception {
        Partido partido = partidoRepo
            .findById(partidoId)
            .orElseThrow(() ->
                new IllegalArgumentException("Partido inexistente")
            );
    ...
    private double[] probabilidades(Partido partido, ModeloPredictivo modelo)
        throws Exception {
        List<Partido> finalizados = partidoRepo.findByEstadoOrderByFechaHoraAsc(
            EstadoPartido.FINALIZADO
        );
        Instances header = FeatureExtractor.buildHeader();
        DenseInstance inst = FeatureExtractor.instancia(
            partido,
            finalizados,
            header
        );
    ...
    private ModeloPredictivo modeloActivo() {
        ModeloPredictivo modelo = modeloRepo
            .findByNombre("resultado_partido")
            .stream()
            .max(Comparator.comparingInt(ModeloPredictivo::getVersion))
            .orElseThrow(() ->
                new IllegalStateException("Sin modelo entrenado")
            );

        System.out.println("MODELO VERSION: " + modelo.getVersion());
        System.out.println("RUTA: " + modelo.getRuta());

        return modelo;
    }

    private void persistir(Partido partido, int version, double[] probs) {
        Prediccion pred = prediccionRepo
            .findByPartidoIdAndModeloVersion(partido.getId(), version)
    ...
    private Map<String, Object> salida(
        Partido partido,
        int version,
        double[] probs
    ) {
        ...
        for (int i = 0; i < probs.length && i < LABELS.length; i++) {
            probabilidades.put(
                LABELS[i],
                Math.round(probs[i] * 10000.0) / 10000.0
            );
        }
        return Map.of(
            "partido_id",
    ...
    public Map<String, Object> rendimientoJugador(Long jugadorId)
        throws Exception {
        ModeloPredictivo modelo = modeloRepo
            .findByNombre("rendimiento_jugador")
            .stream()
            .max(Comparator.comparingInt(ModeloPredictivo::getVersion))
            .orElseThrow(() ->
                new IllegalStateException(
                    "Sin modelo entrenado para rendimiento"
                )
            );

        double goles = ejRepo
            .findByJugadorIdAndMetrica(jugadorId, "goles")
            .map(s -> s.getValor())
            .orElse(0.0);
        ...
        LinearRegression reg = (LinearRegression) SerializationHelper.read(
            modelo.getRuta()
        );
