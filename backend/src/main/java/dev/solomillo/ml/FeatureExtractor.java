...
public final class FeatureExtractor {
    ...
    public static Instances buildHeader() {
        ...
        data.setClassIndex(N_FEATURES);
        return data;
    }

    public static double[] features(Partido objetivo, List<Partido> finalizados) {
        Equipo local = objetivo.getEquipoLocal();
        Equipo visit = objetivo.getEquipoVisitante();
        LocalDateTime corte = objetivo.getFechaHora();

        List<Partido> previos = finalizados.stream()
                .filter(p -> corte == null || p.getFechaHora() == null || p.getFechaHora().isBefore(corte))
        ...
        double[] f = new double[N_FEATURES + 1];
        f[0] = eloDiff;
        f[1] = fifaDiff;
        f[2] = fLocal[0];
        f[3] = fVisit[0];
        ...
        f[7] = fVisit[2];
        f[8] = h2h;
        f[9] = etiqueta(objetivo);
        return f;
    }

    public static double etiqueta(Partido p) {
        if (p.getGolesLocal() == null || p.getGolesVisitante() == null) return 1;
        int gl = p.getGolesLocal(), gv = p.getGolesVisitante();
        return gl > gv ? 0 : gl == gv ? 1 : 2;
    }

    public static DenseInstance instancia(Partido objetivo, List<Partido> finalizados, Instances header) {
    ...
    private static double[] formaYGoles(Equipo eq, List<Partido> previos) {
        ...
        for (int i = previos.size() - 1; i >= 0 && jugados.size() < VENTANA; i--) {
            Partido p = previos.get(i);
            if (esDe(eq, p)) jugados.add(p);
        }
        if (jugados.isEmpty()) return new double[]{1.0, 1.0, 1.0};

        double pts = 0, gf = 0, gc = 0;
        ...
        int n = jugados.size();
        return new double[]{pts / n, gf / n, gc / n};
    }

    private static double h2h(Equipo local, Equipo visit, List<Partido> previos) {
        double saldo = 0;
        for (Partido p : previos) {
            boolean lv = involucra(p, local, visit);
            if (!lv) continue;
            boolean localEsLocal = p.getEquipoLocal().getId().equals(local.getId());
    ...
    private static boolean esDe(Equipo eq, Partido p) {
        return p.getEquipoLocal().getId().equals(eq.getId())
                || p.getEquipoVisitante().getId().equals(eq.getId());
    }

    private static boolean involucra(Partido p, Equipo a, Equipo b) {
        Long la = p.getEquipoLocal().getId(), lv = p.getEquipoVisitante().getId();
        return (la.equals(a.getId()) && lv.equals(b.getId()))
