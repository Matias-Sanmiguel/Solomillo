...
public class MlController {
    ...
    public MlController(MlTrainer t, MlPredictor p, MetricsService metrics,
                        ModeloPredictivoRepository r, PartidoRepository partidoRepo,
                        EquipoRepository equipoRepo, EloHistorialRepository eloHistRepo,
                        AuditService au) {
        this.trainer = t;
        this.predictor = p;
    ...
    public Map<String, Object> entrenar(@AuthenticationPrincipal String email) {
        try {
            ...
            return Map.of("version", m.getVersion(), "accuracy", m.getAccuracy(),
                    "log_loss", m.getLogLoss(), "brier", m.getBrier(), "activo", m.isActivo());
    ...
    public List<Map<String, Object>> modelos() {
        return modeloRepo.findAll().stream()
                ...
                .map(m -> Map.<String, Object>of("version", m.getVersion(), "tipo", m.getTipo(),
                        "accuracy", m.getAccuracy(), "log_loss", m.getLogLoss(),
                        "brier", m.getBrier(), "activo", m.isActivo()))
                .toList();
    }
    ...
    public List<Map<String, Object>> tablero() {
        ...
        proximos.addAll(partidoRepo.findByEstadoOrderByFechaHoraAsc(EstadoPartido.EN_VIVO));
        proximos.addAll(partidoRepo.findByEstadoOrderByFechaHoraAsc(EstadoPartido.PROGRAMADO));
    ...
    public List<Map<String, Object>> elo() {
        return equipoRepo.findAll().stream()
                .sorted(Comparator.comparingDouble((dev.solomillo.domain.Equipo e) ->
                        e.getElo() != null ? e.getElo() : 0).reversed())
                .map(e -> Map.<String, Object>of(
                        "equipo_id", e.getId(),
                        "nombre", e.getNombre(),
                        "escudo", e.getEscudo() != null ? e.getEscudo() : "",
                        "elo", Math.round((e.getElo() != null ? e.getElo() : 0) * 10.0) / 10.0,
                        "puntos_fifa", e.getPuntosFifa() != null ? e.getPuntosFifa() : 0))
                .toList();
    }

    @GetMapping("/elo/{equipoId}/historial")
    public List<Map<String, Object>> eloHistorial(@PathVariable Long equipoId) {
        return eloHistRepo.findByEquipoIdOrderByFechaAsc(equipoId).stream()
                .map(h -> Map.<String, Object>of(
                        "fecha", h.getFecha().toString(),
                        "elo", Math.round(h.getElo() * 10.0) / 10.0))
                .toList();
    }
}
