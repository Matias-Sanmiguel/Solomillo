  ...
  return (
    <div>
      {enVivo.length > 0 && (
        <div className="mb-6">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="chip">{enVivo.length} partido{enVivo.length !== 1 ? "s" : ""}</span>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {enVivo.map((p) => {
              ...
              return (
                <div key={p.id} className="card border-good/30 bg-panel2/60 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    {pred && (
                      <span className="chip">
                        {favorito(pred.probabilidades).label} · {Math.round(favorito(pred.probabilidades).conf * 100)}%
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={() => demoPost(`/demo/gol/${p.id}?equipo=visitante`)}
                      className="chip cursor-pointer hover:border-visit/60 hover:text-visit transition"
                    >
                      ⚽ Gol visita
                    </button>
                    <button
                      onClick={() => demoPost(`/demo/tarjeta/${p.id}?color=amarilla`)}
      ...
      <div className="mb-5 flex items-baseline justify-between">
        ...
        <span className="chip">{preds.filter(pr => partidos.find(p => p.id === pr.partido_id)?.estado === "PROGRAMADO").length} predicciones</span>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {preds
          .filter((pred) => partidoOf.get(pred.partido_id)?.estado === "PROGRAMADO")
          .map((pred) => {
